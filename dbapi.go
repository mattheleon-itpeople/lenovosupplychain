...
//////////////////////////////////////////////////////////////////////////////////////////////////
const (
	BIT string = "BasicInfoTable"
)

var recType = []string{BIT}
...
//////////////////////////////////////////////////////////////////////////////////////////////////
var Objects = []string{BIT}

var ObjectMap = map[string]int{
	BIT: 3}

var logger = shim.NewLogger("dbapi")
...
func CheckRecType(rt string) bool {
	for _, val := range recType {
		if val == rt {
			logger.Debugf("CheckRequestType() : Valid Request Type , val : ", val, rt, "\n")
			return true
		}
	}
	logger.Errorf("CheckRequestType() : Invalid Request Type , val : ", rt, "\n")
	return false
}
...
func InitObject(stub shim.ChaincodeStubInterface, objectType string, keys []string) error {
	...
	logger.Debugf(">> Not Implemented Yet << Initializing Object : ", objectType, " Keys: ", keys)
	return nil
}
...
func DeleteObject(stub shim.ChaincodeStubInterface, objectType string, keys []string) error {
	...
	if err != nil {
		logger.Errorf("DeleteObject() : Error deleting Object into State Database %s", err)
		return err
	}
	logger.Infof("DeleteObject() : Object : %s Key : %s", objectType, compositeKey)
...
func DeleteAllObjects(stub shim.ChaincodeStubInterface, objectType string) error {
	...
	err := stub.DelState(compositeKey)
	if err != nil {
		logger.Errorf("DeleteAllObjects() : Error deleting all Object into State Database %s", err)
		return err
	}
	logger.Debugf("DeleteObject() : ", "Object : ", objectType, " Key : ", compositeKey)
...
func ReplaceObject(stub shim.ChaincodeStubInterface, objectType string, keys []string, objectData []byte) error {
	...
	err = stub.PutState(compositeKey, objectData)
	if err != nil {
		logger.Errorf("ReplaceObject() : Error replacing Object in State Database %s", err)
		return err
	}

	logger.Debugf("ReplaceObject() : - end init object ", objectType)
	return nil
}
...
func QueryObject(stub shim.ChaincodeStubInterface, objectType string, keys []string) ([]byte, error) {
	...
	if err != nil {
		logger.Errorf("QueryObject() : Error Querying Object in State Database %s", err)
		return nil, err
	}
...
func QueryObjectWithProcessingFunction(stub shim.ChaincodeStubInterface, objectType string, keys []string, fname func(shim.ChaincodeStubInterface, []byte, []string) error) ([]byte, error) {
	...
	if err != nil {
		logger.Errorf("QueryLedger() : Cannot execute  : ", fname)
...
func GetKeyList(stub shim.ChaincodeStubInterface, args []string) (shim.StateQueryIteratorInterface, error) {
	...
	for i = 0; resultsIterator.HasNext(); i++ {
		...
		logger.Debugf("GetList() : my Value : ", myCompositeKey)
	}
...
func GetQueryResultForQueryString(stub shim.ChaincodeStubInterface, queryString string) ([]byte, error) {
	...
	logger.Debugf("GetQueryResultForQueryString() : getQueryResultForQueryString queryString:\n%s\n", queryString)
	...
	logger.Debugf("GetQueryResultForQueryString(): getQueryResultForQueryString queryResult:\n%s\n", buffer.String())
...
func GetList(stub shim.ChaincodeStubInterface, objectType string, keys []string) (shim.StateQueryIteratorInterface, error) {
	...
	logger.Debugf("GetList(): Retrieving Objects into an array")
	if err != nil {
		logger.Errorf("GetList(): Retrieving Objects into an array failed with error: %v", err)
		return nil, err
	}
